...
{
	return (rand() - (RAND_MAX / 2)) & (((1UL << percpu_counter_tree_get_depth(counter)) * batch_size) - 1);
}
...
static
void test_counter_negative(unsigned long batch_size, enum percpu_counter_tree_type type)
{
...
static
void test_counter_incdec(unsigned long batch_size, enum percpu_counter_tree_type type)
{
	struct percpu_counter_tree *counter;
	long cnt = 0, i;
...
static
void test_counter_random(unsigned long batch_size, enum percpu_counter_tree_type type)
{
	struct percpu_counter_tree *counter;
	long cnt = 0, i;
...
static
void test_counter_random_cpu_hop(unsigned long batch_size, enum percpu_counter_tree_type type)
{
	struct percpu_counter_tree *counter;
	long cnt = 0, i;
	int ret;
...
int main(void)
{
	...
	printf("Type byte\n");
	test_counter_negative(32, PERCPU_COUNTER_TREE_TYPE_BYTE);
	test_counter_incdec(32, PERCPU_COUNTER_TREE_TYPE_BYTE);
	test_counter_random(32, PERCPU_COUNTER_TREE_TYPE_BYTE);
	test_counter_random_cpu_hop(32, PERCPU_COUNTER_TREE_TYPE_BYTE);

	printf("Type long\n");
	test_counter_negative(32, PERCPU_COUNTER_TREE_TYPE_LONG);
	test_counter_incdec(32, PERCPU_COUNTER_TREE_TYPE_LONG);
	test_counter_random(32, PERCPU_COUNTER_TREE_TYPE_LONG);
	test_counter_random_cpu_hop(32, PERCPU_COUNTER_TREE_TYPE_LONG);
